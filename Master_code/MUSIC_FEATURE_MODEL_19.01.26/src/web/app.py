    ...
    try:
        ...
        def on_status(status_data):
            ...
            if state.data_recorder.is_recording():
                ...
                state.data_recorder.record_sample(user_amp, original_pos, final_pos)
        ...
        # Set initial amplitude from Bluetooth remote if connected
        if state.bluetooth.is_connected():
            state.controller.set_user_amplitude(state.bluetooth_user_amplitude)
        else:
            # Use slider amplitude (default 0.3)
            state.controller.set_user_amplitude(state.bluetooth_user_amplitude)
...
def _save_study_data_background(trial, data_recorder, user_study):
    ...
    try:
        ...
        socketio.emit('status', {
            'message': f'Data saved: {base_name}',
            'type': 'success'
        })
    except Exception as e:
        socketio.emit('status', {
...
def handle_song_latency(song_id):
    ...
    try:
        ...
        socketio.emit('status', {
            'message': f'Saved latency {latency_ms:.0f}ms for {song.metadata.name}',
            'type': 'success'
        })
...
def bluetooth_reset_baseline():
    """Reset the encoder baseline (sets amplitude to 0.5)."""
    try:
