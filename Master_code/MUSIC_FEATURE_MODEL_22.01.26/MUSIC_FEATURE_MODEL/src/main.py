...
def play_song(song_id: str, odrv0, latency_offset_s: float = 0.035) -> None:
    ...
    # Check if trajectory has velocity feedforward
    from core import TrajectoryPlayer
    ...
    print(f"Velocity feedforward: {'enabled' if has_feedforward else 'not available (regenerate trajectory)'}")
    print(f"{'='*60}\n")
