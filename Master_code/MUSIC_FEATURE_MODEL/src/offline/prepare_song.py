...
def interactive_prepare():
    ...
    print("\nMotor protection options:")
    motor_rpm_max = prompt_float("Motor RPM limit for amplitude attenuation", 5500.0, 1000.0, 10000.0)
    enable_attenuation = prompt_yes_no("  Enable amplitude attenuation (prevents motor over-speed)", True)
    ...
    for audio_file in selected_files:
        ...
        try:
            song_id = prepare_song(
                ...
                enable_amplitude_attenuation=enable_attenuation
            )
    ...
    try:
        ...
        extra_data = {
            ...
            'trajectory': {
                ...
                'half_freq_complex': half_freq_complex,
                'motor_rpm_max': motor_rpm_max,
                'amplitude_attenuation_enabled': enable_amplitude_attenuation
            }
        }
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=5500.0,
        help="Maximum safe motor RPM for amplitude attenuation (default: 5500)"
    )
    parser.add_argument(
        "--no-attenuation",
        action="store_true",
        help="Disable amplitude attenuation (not recommended for high BPM songs)"
    )
    ...
    try:
        song_id = prepare_song(
            ...
            motor_rpm_max=args.motor_rpm_max,
            enable_amplitude_attenuation=not args.no_attenuation
        )
