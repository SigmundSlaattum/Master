...
class DataRecorder:
    ...
    def plot_synchronization(self, save_filename: Optional[str] = None,
        ...
        try:
            ...
            # Find peaks in music sinusoid (local maxima)
            from scipy.signal import find_peaks
            music_peak_indices, _ = find_peaks(music_sinusoid, height=0.5 * actual_amplitude)

            # Find peaks in motor position (local maxima)
            motor_peak_indices, _ = find_peaks(actual_centered, height=0.5 * actual_amplitude)

            # For each music peak, find the nearest motor peak and calculate timing offset
            peak_times = []  # Time of each music peak
            peak_offsets_ms = []  # Offset in ms (positive = motor is late)

            for music_idx in music_peak_indices:
                music_peak_time = times[music_idx]

                if len(motor_peak_indices) > 0:
                    # Find nearest motor peak
                    motor_peak_times = times[motor_peak_indices]
                    time_diffs = motor_peak_times - music_peak_time
                    nearest_idx = np.argmin(np.abs(time_diffs))
                    offset_ms = time_diffs[nearest_idx] * 1000  # Convert to ms (positive = motor late)

                    peak_times.append(music_peak_time)
                    peak_offsets_ms.append(offset_ms)

            peak_times = np.array(peak_times)
            peak_offsets_ms = np.array(peak_offsets_ms)

            # Calculate peak offset statistics
            if len(peak_offsets_ms) > 0:
                mean_offset_ms = np.mean(peak_offsets_ms)
                std_offset_ms = np.std(peak_offsets_ms)
            else:
                mean_offset_ms = 0.0
                std_offset_ms = 0.0
            ...
            metrics_text = (f'BPM: {self.bpm:.1f} ({music_freq:.2f} Hz)\n'
                          ...
                          f'Mean Peak Offset: {mean_offset_ms:.1f} ms\n'
                          f'Peak Offset Std: {std_offset_ms:.1f} ms')
            props = dict(boxstyle='round', facecolor='wheat', alpha=0.8)
            ax1.text(0.02, 0.98, metrics_text, transform=ax1.transAxes, fontsize=10,
                    verticalalignment='top', bbox=props, family='monospace')

            # Plot 2: Peak timing offset over time
            if len(peak_times) > 0:
                ax2.plot(peak_times, peak_offsets_ms, 'go-', linewidth=1.5, markersize=6,
                        label='Peak Timing Offset')
                ax2.axhline(y=0, color='k', linestyle='-', linewidth=1, alpha=0.5, label='Perfect sync')
                ax2.axhline(y=mean_offset_ms, color='b', linestyle='--', linewidth=1.5,
                           alpha=0.7, label=f'Mean ({mean_offset_ms:.1f} ms)')
                # Add ±1 std deviation band
                ax2.fill_between(peak_times, mean_offset_ms - std_offset_ms,
                                mean_offset_ms + std_offset_ms, alpha=0.2, color='blue',
                                label=f'±1 Std ({std_offset_ms:.1f} ms)')
            else:
                ax2.text(0.5, 0.5, 'No peaks detected', transform=ax2.transAxes,
                        ha='center', va='center', fontsize=12)

            ax2.set_ylabel('Timing Offset (ms)', fontsize=12, fontweight='bold')
            ax2.set_xlabel('Time (seconds)', fontsize=12, fontweight='bold')
            ax2.set_title('Peak Synchronization (Positive = Motor Late, Negative = Motor Early)', fontsize=11)
            ax2.grid(True, alpha=0.3)
    ...
    def export_sync_data(self, filename: str, song_name: str = "", pattern_type: str = ""):
        ...
        try:
            ...
            # Find peaks and calculate timing offsets
            from scipy.signal import find_peaks
            music_peak_indices, _ = find_peaks(music_sinusoid, height=0.5 * actual_amplitude)
            motor_peak_indices, _ = find_peaks(actual_centered, height=0.5 * actual_amplitude)
            ...
            with open(filename, 'w', newline='') as f:
                ...
                writer.writerow([f'# Mean Peak Offset: {mean_offset_ms:.2f} ms'])
                writer.writerow([f'# Peak Offset Std: {std_offset_ms:.2f} ms'])
                writer.writerow([f'# Number of Peaks: {len(peak_times)}'])
                writer.writerow([])

                # Write peak timing offset data
                writer.writerow(['# Peak Timing Offsets (Positive = Motor Late)'])
                writer.writerow(['Peak Time (s)', 'Timing Offset (ms)'])
                for i in range(len(peak_times)):
                    writer.writerow([peak_times[i], peak_offsets_ms[i]])

                writer.writerow([])

                # Write full time series data
                writer.writerow(['# Full Time Series Data'])
                writer.writerow(['Time (s)', 'Music Sinusoid', 'Actual Position (centered)'])
