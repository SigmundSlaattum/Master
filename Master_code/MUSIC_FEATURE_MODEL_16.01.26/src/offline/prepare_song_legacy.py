...
def prepare_song_legacy(
    ...
    trajectory_dynamic_path = song_dir / "trajectory_dynamic.npy"
    ...
    try:
        ...
        print(f"\n[2/5] Extracting features (LEGACY sliding window)...")
        ...
        # Step 3: Generate trajectories (both fixed and dynamic tempo)
        print(f"\n[3/5] Generating fixed tempo trajectory...")
        ...
        print(f"\n[4/5] Generating dynamic tempo trajectory...")
        ...
        # Step 5: Register in index and save metadata
        print(f"\n[5/5] Registering song...")
        manager.add_song(
            name=name,
            artist=artist,
...
def interactive_prepare():
    ...
    # Get prepared songs from standard library
    prepared_songs = list_prepared_songs()

    if not prepared_songs:
        print(f"\nNo prepared songs found in: {get_prepared_songs_dir()}")
        print("Please prepare songs using prepare_song.py first.")
        return 1

    # Display songs and let user select
    song_display = [f"{s['name']} - {s['artist']} ({s['bpm']:.0f} BPM)" for s in prepared_songs]
    selected_indices = prompt_choice(
        f"Prepared songs in standard library ({get_prepared_songs_dir()}):",
        song_display,
        allow_multiple=True
    )
    ...
    selected_songs = [prepared_songs[i] for i in selected_indices]
    print(f"\nSelected {len(selected_songs)} song(s) for LEGACY preparation.")
    ...
    for song in selected_songs:
        print("\n" + "=" * 60)
        print(f"Preparing: {song['name']} - {song['artist']}")
        print("=" * 60)

        # Use existing metadata from prepared song
        name = song['name']
        artist = song['artist']
        existing_bpm = song['bpm']

        # Show existing metadata and allow changes
        print("\nSong Metadata (from standard library):")
        name = prompt_text("  Song name", name)
        artist = prompt_text("  Artist", artist)

        # Confirm BPM (use existing from standard library)
        print(f"\n  Existing BPM: {existing_bpm:.1f}")
        print(f"  Options: Enter=accept, h=halve ({existing_bpm/2:.0f}), d=double ({existing_bpm*2:.0f}), or type a number")
        final_bpm = prompt_bpm(existing_bpm)
        ...
        # Prepare the song using audio from standard library
        try:
