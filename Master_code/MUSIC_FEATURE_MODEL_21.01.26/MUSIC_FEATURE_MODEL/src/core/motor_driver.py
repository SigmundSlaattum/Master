...
class MotorDriver:
    ...
    def set_position(self, position: float) -> None:
        """
        Set motor position (without velocity feedforward).
    ...
    def set_position_with_velocity(self, position: float, velocity: float) -> None:
        """
        Set motor position with velocity feedforward.

        Velocity feedforward improves tracking performance by telling
        the controller the expected velocity, reducing phase lag
        especially during fast direction changes.

        This is the hot path called at 60Hz.

        Args:
            position: Target position in motor turns
            velocity: Feedforward velocity in turns/second
        """
        self._last_command = position
        ...
        try:
            # Set both position and velocity feedforward
            # Order matters: set velocity first, then position
    ...
    def get_last_velocity_command(self) -> float:
        """Get the last commanded velocity (feedforward)."""
